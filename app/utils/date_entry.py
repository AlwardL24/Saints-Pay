...
class Calendar(Tkinter.Toplevel):
    ...
    def __init__(
        ...
        self.bind(
            "<FocusIn>", lambda event: self.event_generate("<<DatePickerFocusIn>>")
        )
        self.bind(
            "<FocusOut>", lambda event: self.event_generate("<<DatePickerFocusOut>>")
        )
        ...
        lbtn = ttk.Button(
            hframe, style="Calendar.L.TButton", command=self._on_press_left_button
        )
        lbtn.pack(side=LEFT)
        ...
        rbtn = ttk.Button(
            hframe, style="Calendar.R.TButton", command=self._on_press_right_button
        )
        rbtn.pack(side=LEFT)
        ...
        for i, day_of_the_week in enumerate(days_of_the_week):
            Tkinter.Label(self, text=day_of_the_week, background="grey90").grid(
        ...
        for i in range(6):
            for j in range(7):
                ...
                label.bind(
                    "<Leave>", lambda event: event.widget.configure(background="white")
                )
    ...
    def _build_calendar(self, year, month):
        if not (self._year == year and self._month == month):
            ...
            for i in range(len(cal)):
                week = cal[i]
                fmt_week = [day if day else "" for day in week]
    ...
    def _find_label_coordinates(self, date):
        ...
        return divmod(
            (first_weekday_of_the_month - self._cal.firstweekday) % 7 + date.day, 7
        )
...
class Datepicker(ttk.Entry):
    def __init__(
        ...
        **kwargs,
        ...
        ttk.Entry.__init__(
            self, master, textvariable=self.date_var, **entry_config, **kwargs
        )
        ...
        self.calendar_frame.withdraw()

        self.bind_all("<1>", self._on_click, "+")

        # self.bind("<Destroy>", lambda _: self.unbind("<1>", _all_on_click_binding_id))
        # For some reason, unbinding does not work, preventing any new date pickers from ever being able to bind to the same event again. Instead there is a guard in the _on_click method to check if the widget still exists before doing anything.
        ...
        self.calendar_frame.bind(
            "<<DatePickerFocusOut>>", lambda event: self._on_calendar_focus_out()
        )
        ...
        # CTRL + SHIFT + PAGE UP: Move to the previous year.
        self.bind(
            "<Control-Shift-Prior>", lambda event: self.calendar_frame.prev_year()
        )
        ...
        # CTRL + RIGHT: Move to the next day.
        self.bind(
            "<Control-Right>", lambda event: self.calendar_frame.select_next_day()
        )

        # CTRL + UP: Move to the previous week.
        self.bind(
            "<Control-Up>", lambda event: self.calendar_frame.select_prev_week_day()
        )

        # CTRL + DOWN: Move to the next week.
        self.bind(
            "<Control-Down>", lambda event: self.calendar_frame.select_next_week_day()
        )
        ...
        # CTRL + HOME: Move to the current month.
        self.bind(
            "<Control-Home>", lambda event: self.calendar_frame.select_current_date()
        )
    ...
    def set_date_from_calendar(self):
        if self.is_calendar_visible:
            ...
            if selected_date is not None:
                if selected_date.strftime(self.date_format) == self.date_var.get():
                    return
    ...
    @property
    def _is_calendar_visible(self):
        return self.calendar_frame.winfo_viewable()
    ...
    def current_date(self):
        try:
            # oddly enough, datetime.datetime.strftime() works fine with non-zero-padded formatting but not strptime(), so we need to replace all "%-" in the format string with "%"
            date = datetime.datetime.strptime(
                self.date_var.get(), self.date_format.replace("%-", "%")
            )
            return date
        except ValueError:
            return None
    ...
    def show_calendar(self):
        if not self._is_calendar_visible:
            self.calendar_frame.geometry(
                f"+{self.winfo_rootx()}+{self.winfo_rooty() + self.winfo_height()}"
            )
            self.calendar_frame.deiconify()
    ...
    def _on_selected_date(self, date):
        if date.strftime(self.date_format) == self.date_var.get():
            return
    ...
    def _on_click(self, event):
        ...
        if str_widget == str(self):
            ...
            if (
                not str_widget.startswith(str(self.calendar_frame))
                and self._is_calendar_visible
            ):
                self.hide_calendar()
...
if __name__ == "__main__":
    ...
    Datepicker(main, dateformat="%-d/%m/%Y").pack(anchor="w")
